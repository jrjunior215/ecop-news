...
export async function scrapeDarkReading() {
  cron.schedule("0 */3 * * *", async () => {
    try {
      const url = "https://www.securityweek.com/";
      ...
      if (response.status === 200) {
        ...
        removeElementsByClass($, ".sw-home-ads.zoxrel.text-center.sw-ads-top");
        removeElementsByClass(
          $,
          ".wp-block-column.is-layout-flow.wp-block-column-is-layout-flow"
        );
        removeElementsByClass($, ".wp-block-column.sw-home-center.is-layout-flow.wp-block-column-is-layout-flow");
        removeElementsByClass($, ".wp-block-group__inner-container");
